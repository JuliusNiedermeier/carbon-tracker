...
export const CategoryFooter: FC<EmissionFactorFinderDisplayHeaderContext> = (props) => {
  const { selectedFactorInfo, setSelectedFactorID } = useFactorFinder();

  return (
    <Cell className="flex-1 gap-4 pl-4" padding={false}>
      <Button variant="secondary" className="gap-2 hover:text-red-600" onClick={() => setSelectedFactorID(null)}>
        <X className="" size="16" />
        Unlink factor
      </Button>
