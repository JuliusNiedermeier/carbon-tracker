...
WITH RECURSIVE corporate_group AS (
    SELECT company.id AS root_company_id, company.id, company.parent_company_id, company.slug, company.name, ARRAY[id] AS members
    FROM company
    WHERE company.parent_company_id IS NULL
    
    UNION ALL
    
    SELECT corporate_group.root_company_id, company.id, company.parent_company_id, company.slug, company.name, corporate_group.members || company.id
    FROM company
